...
class HomeViewTestCase(TestCase):
    def setUp(self):
        ...
        Post.objects.create(author=self.user, title="Test Post 1", content="Content 1")
        Post.objects.create(author=self.user, title="Test Post 2", content="Content 2")


    def test_home_view(self):
        '''
        Test when Get request is made to the 'blog-home' URL and the 200 Status code is indicating that request recieved successfully
        '''
...
class PostListViewTest(TestCase):
    def setUp(self):
        '''
        Create a user and 12 posts with timestamps ranging from 1 day ago to 12 days ago.
        '''
        self.user = User.objects.create_user(username='myUsername', password='myPassword')
        ...
        for i in range(1, 13):
            ...
            Post.objects.create(author=self.user, title=f"Test Post {i}", content=f"Content {i}")


    def test_ordering(self):
        '''
        Test that the posts are ordered correctly based on the date_posted field.
        
        1. Make a GET request to the 'blog-home' URL.
        2. Get the list of posts from the response context.
        3. Iterate over the list of posts and check if the posts are ordered correctly based on the date_posted field 
        '''
        response = self.client.get(reverse('blog-home'))
    ...
    def test_context_object_name(self):
        '''
        1. Make get request to the 'blog-home' URL and retrieve the post list in context_posts.
        ...
        database_posts = Paginator(Post.objects.all().order_by('-date_posted'), 6)
    ...
    def test_pagination(self):
        '''
        1. Make a GET request to the 'blog-home' URL and check if pagination is True.
...
class post_detail(TestCase):
    def setUp(self):
        ...
        #mock_context = Mock()
        self.comments = [
            Comment.objects.create(author=self.user, post=self.post, body="Comment 1"),
            Comment.objects.create(author=self.user, post=self.post, body="Comment 2"),
            Comment.objects.create(author=self.user, post=self.post, body="Comment 3"),
        ]
    ...
    def test_response_for_existing_posts(self):
        '''
        Test when a GET request is made to the 'post_detail' URL with a valid slug,
        the response status code is 200, indicating a successful request, and the
    ...
    def test_response_for_non_exising_posts(self):
        '''
        Test when GET request is made to 'post_detail' URL with a slug that does not correspond to any exising post.
        The 404 status code is indicating then that the post was not found
        '''
        response = self.client.get(reverse('post_detail', kwargs={'slug': 'non-exisiting-slug'}))
    ...
    def test_comment_submission_with_approval_message(self):
        ...
        1. Log in a user.
        2. Simulate a POST request to the comment submission URL with test comment data.
        3. Check if the response status code is 200 or 201, indicating a successful submission.
        ...
        response = self.client.post(reverse('post_detail', kwargs={'slug':self.post.slug}), {
            'body': 'Test comment',
        },follow = True) # follow redirect after submission

        self.assertIn(response.status_code, [200, 201])

        comment = Comment.objects.filter(post=self.post, body='Test comment').first()
        self.assertIsNotNone(comment)

        self.assertIn(messages.SUCCESS, [msg.level for msg in response.context['messages']])


    def test_form_validation(self):
        '''
        Test form validation by submitting invalid data to the comment submission form.
        '''
        self.client.force_login(self.user)
        ...
        self.assertEqual(comment_form.errors['body'], ['This field is required.'])
    ...
    def test_comment_ordering(self):
        ...
        """
        comment1 = Comment.objects.create(post=self.post, author=self.user, body="Comment 1")
        comment2 = Comment.objects.create(post=self.post, author=self.user, body="Comment 2")
    ...
    def test_total_likes_count(self):
        ...
        self.post.likes.add(User.objects.create_user(username='anotheruser', password='password456'))

        response = self.client.get(reverse('post_detail', kwargs={'slug': self.post.slug}))

        # Check if the total likes count matches the expected value (2)
    ...
    def test_context(self):
        ...
        response_comments = sorted(response.context['comments'], key=lambda comment: comment.body)
        self_comments = sorted(self.comments, key=lambda comment: comment.body)


        self.assertEqual([comment.body for comment in response_comments], [comment.body for comment in self_comments])


        self.assertTrue(hasattr(mock_context, 'comment_form'))
        self.assertEqual(response.context['comment_count'], len(self.comments))


        self.assertTrue(hasattr(mock_context, 'comment_form'))  # Verify attribute exists
...
class create_new_post(TestCase):
    ...
    def test_post_creation(self):
        ...
        1. Log in a user
        2. Create a dictionary `post_data` containing test data for the new post.
        3. Send a POST request to the `new_post` view with the `post_data`.
        4. Assert that the response status code is 302 (indicating a redirect after successful creation).
        5. Assert that a new Post object was created and saved in the database.
        '''
        self.client.force_login(self.user)
        post_data = {
            'title': 'Test Post',
    ...
    def test_form_validation_error(self):
        '''
        Test that the form validation works correctly when invalid data is submitted.
    
        1. Log in a user.
        2. Modify the post data with invalid data.
        3. Send a POST request with the invalid data to simulate form submission.
        4. Ensure that the response status code is 200, indicating a failed form submission.
        5. Instantiate the form with the invalid data and test that it's not valid.
        '''
        self.client.force_login(self.user)
        invalid_post_data = {}
    ...
    def test_successful_submission_and_message(self):
        ...
        self.assertContains(redirected_response, 'Your post was successfully submitted!')
    ...
    def test_error_message(self):
        ...
        3. Retrieve template name and empty dictonary saved to invalid_post_data
        4. Verify existance of error message
        '''
        self.client.force_login(self.user)
        invalid_post_data = {}
        response = self.client.post(reverse('new_post'), invalid_post_data)
        self.assertContains(response, 'There was an error submitting your post.')
...
class update_post(TestCase):
    def setUp(self):
        ...
        '''
        self.user = User.objects.create(username='Myusername', password="Mypassword")
    ...
    def test_author_access_update_form(self):
        ...
        response = self.client.get(reverse('update_post', kwargs={'slug': self.post.slug}), follow=True)
    ...
    def test_non_author_access_update_form(self):
        ...
        '''
        other_ueser = User.objects.create(username='Otherusername', password='Otherpassword')
    ...
    def test_update_post_valid_data(self):
        ...
        response = self.client.post(reverse('update_post', kwargs={'slug': self.post.slug}), updated_data, follow=True)
        
        # Check if the response is a redirect
        self.assertRedirects(response, reverse('post_detail', kwargs={'slug': 'updated-title'}))
    ...
    def test_update_post_invalid_data(self):
        ...
        response = self.client.post(reverse('update_post', kwargs={'slug': self.post.slug}), invalid_data, follow=True)

        # Check if the response status code is 200
        self.assertEqual(response.status_code, 200)
        
        # Verify that error messages are displayed
        self.assertFormError(response, 'post_form', 'title', 'This field is required.')

    
    def test_update_post_get_request(self):
        '''
    
        Test to ensure that the update post form is accessible via a GET request.
        ...
        response = self.client.get(reverse('update_post', kwargs={'slug': self.post.slug}))
        
        self.assertEqual(response.status_code, 200)
        
        self.assertTemplateUsed(response, 'blog/update_post.html')
        
        self.assertEqual(response.context['post_form'].instance.title, self.post.title)
        self.assertEqual(response.context['post_form'].instance.content, self.post.content)
...
class DeletePostTestCase(TestCase):
    ...
    def test_user_not_logged_in_cannot_delete_post(self):
        ...
        '''
        response = self.client.post(reverse('delete_post', args=[self.post.slug]))
        self.assertRedirects(response, reverse('login') + '?next=' + reverse('delete_post', args=[self.post.slug]))
...
class edit_comment(TestCase):
    def setUp(self):
        ...
        '''
        self.user = User.objects.create_user(username='Myusername', password='Mypassword')
        self.other_user = User.objects.create_user(username='otherusername', password='otherpassword')
        self.post = Post.objects.create(title='Test Post', content='Test Content', author=self.user)
        self.comment = Comment.objects.create(body='Test Comment', author=self.user, post=self.post)
    ...
    def test_user_logged_in_can_edit_own_comment(self):
        ...
        response = self.client.post(reverse('edit_comment', args=[self.post.slug, self.comment.id]), data={'body': new_body})
        self.assertRedirects(response, reverse('post_detail', args=[self.post.slug]))
        
        # Assert comment is updated
        self.assertTrue(Comment.objects.filter(pk=self.comment.id, body=new_body).exists())
    ...
    def test_user_not_logged_in_cannot_edit_comments(self):
        ...
        '''
        self.client.logout()
        response = self.client.post(reverse('edit_comment', args=[self.post.slug, self.comment.id]))
        
        # Assert that the response redirects to the login page
        self.assertRedirects(response, reverse('login') + '?next=' + reverse('edit_comment', args=[self.post.slug, self.comment.id]))
    ...
    def test_user_not_author_cannot_edit_comment(self):
        ...
        self.assertTrue(Post.objects.filter(pk=self.comment.id).exists())  # Placeholder, update based on your logic
    ...
    def test_comment_edited_successfully(self):
        ...
        response = self.client.post(reverse('edit_comment', args=[self.post.slug, self.comment.id]), data={'body': updated_body})
        self.assertRedirects(response, reverse('post_detail', args=[self.post.slug]))
...
class delete_comment(TestCase):
    ...
    def test_user_logged_in_can_delete_own_comment(self):
        ...
        '''
        self.client.login(username='Myusername', password='Mypassword')
        response = self.client.post(reverse('delete_comment', args=[self.post.slug, self.comment.id]))
        self.assertRedirects(response, reverse('post_detail', args=[self.post.slug]))
    ...
    def test_user_not_logged_in_cannot_delete_comments(self):
        ...
        self.client.logout()
        response = self.client.post(reverse('delete_comment', args=[self.post.slug, self.comment.id]))
        self.assertRedirects(response, reverse('login') + '?next=' + reverse('delete_comment', args=[self.post.slug, self.comment.id]))
...
class LikesView(TestCase):
    ...
    def test_logged_in_user_can_like_post(self):
        ...
        self.assertEqual(response.status_code, 302)  # Redirects to home page after liking
        self.assertIn(self.user, self.post.likes.all())  # User is in the post's likes

    def test_logged_in_user_can_unlike_post(self):
        '''
        Test if a logged in user can unlike a post
        '''
        self.post.likes.add(self.user)  # User likes the post
        self.client.force_login(self.user)
        response = self.client.post(reverse('like_post', kwargs={'pk': self.post.pk}))
        self.assertEqual(response.status_code, 302)  # Redirects to home page after unliking
        self.assertNotIn(self.user, self.post.likes.all())  # User is not in the post's likes
...
class thumbs(TestCase):
    def setUp(self):
        """
        Sets up the initial state for each test. It creates a user and a post,
        and stores the URL for the 'thumbs' view function for the created post.
        """
        self.user = User.objects.create_user(username='Myusername', password='Mypassword')
    ...
    def test_successfull_upvote_unvoted_user(self):
        """
        This test checks the scenario where a user who has not previously voted on a post
        attempts to upvote it. It verifies that the upvote is successfully processed,
        the response status code is 200, and the response JSON contains the 'up' key
        indicating the number of upvotes, without any error messages.
        """
        self.client.login(username='Myusername', password='Mypassword')
        response = self.client.post(self.url, {'action': 'thumbs', 'button': 'thumbsup'})
        self.assertEqual(response.status_code, 200)
        response_json = response.json()
        self.assertIn('up', response_json) # Check if 'up' key exists
        self.assertNotIn('error', response_json) # Ensure 'error' key does not exist

    def test_successful_downvote_unvoted_user(self):
        """
        This test checks the scenario where a user who has not previously voted on a post
        attempts to downvote it. It verifies that the downvote is successfully processed,
        the response status code is 200, and the response JSON contains the 'down' key
        indicating the number of downvotes, without any error messages.
        """
        self.client.login(username='Myusername', password='Mypassword')
        response = self.client.post(self.url, {'action': 'thumbs', 'button': 'thumbsdown'})
    ...
    def test_upvote_by_user_who_already_upvoted(self):
        """
        This test checks the scenario where a user who has already upvoted a post
        attempts to upvote it again. It verifies that the upvote is successfully removed,
        the response status code is 200, and the response JSON contains the 'up' key
        indicating the updated number of upvotes, without any error messages.
        """
        self.client.login(username='Myusername', password='Mypassword')

        self.voted = Vote.objects.create(post=self.post, user=self.user, vote=True)
        response = self.client.post(self.url, {'action': 'thumbs', 'button': 'thumbsup'})
    ...
    def test_downvote_by_user_who_already_downvoted(self):
        """
        This test checks the scenario where a user who has already downvoted a post
        attempts to downvote it again. It verifies that the downvote is successfully removed,
        the response status code is 200, and the response JSON contains the 'down' key
        indicating the updated number of downvotes, without any error messages.
        """
        self.client.login(username='Myusername', password='Mypassword')
